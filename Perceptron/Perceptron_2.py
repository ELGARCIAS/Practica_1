...
class Xarxaneuronal2:
    def __init__(self):
        ...
        # Inicializo pesos y bias con valor 1
        self.pesos_1 = np.ones((self.n_entradas, self.n_internas))
        ...
        self.bias_1 = np.ones((self.n_internas))
        ...
        self.pesos_2 = np.ones((self.n_internas, self.n_salidas))
    ...
    def entrenar(self, entradas, salidas_deseadas):
        for _ in range(1000): # He escogido 1000 ya que segun diferentes fuentes es una cantidad eficiente y suficiente para realizar una tarea asi.
...
#Entrades i sortides
entradas = np.array([[0,1]]) # Me habia dado err
salidas_deseadas = np.array([0,1])

#Entreno la red neuronal
xarxa.entrenar(entradas, salidas_deseadas)

#Test xarxa amb entrada ([0,1])
salidas = xarxa.forward_propagation(entradas)[1]
print(salidas)
